...
#ifndef _WIFIMULTI_GENERIC_IMPL_H_
#define _WIFIMULTI_GENERIC_IMPL_H_ 
    ...
    else 
    {
      ...
      for (int8_t i = 0; i < scanResult; ++i) 
      {
        ...
        if (known) 
        {
          ...
          WFM_LOGDEBUG5("SSID:", ssid_scan.c_str(), ", RSSI:", rssi_scan, ", Secured:", (sec_scan == WIFI_AUTH_OPEN) ? 'n' : 'y');
        } 
        else 
        {
    ...
    if (bestNetwork.ssid) 
    {
      ...
      switch (status) 
      {
        case WL_CONNECTED:
          ...
          WFM_LOGDEBUG3("[run] SSID: ", WiFi.SSID().c_str(), ", IP: ", WiFi.localIP().toString().c_str());
          WFM_LOGDEBUG3("[run] MAC: ",  WiFi.BSSIDstr().c_str(), ", Channel: ", WiFi.channel());
...
#endif    // _WIFIMULTI_GENERIC_IMPL_H_
