...
bool WiFiMulti_Generic::addAP(const char* ssid, const char *passphrase)
{
  ...
  //memcpy(newAP.ssid, ssid, sizeof(ssid));
  // Equivalent to strdup()
  //newAP.ssid = new char[ strlen(ssid) + 1 ];  //malloc(strlen(ptr1)+1);
  ...
  if (passphrase && *passphrase != 0x00) 
  {
    ...
    //newAP.passphrase = new char[ strlen(passphrase) + 1 ];  //malloc(strlen(ptr1)+1);
...
uint8_t WiFiMulti_Generic::run(const uint32_t& connectTimeout)
{
  ...
  else if (scanResult >= 0) 
  {
    ...
    if (bestNetwork.ssid) 
    {
      ...
      WFM_LOGINFO3("[run] Connecting SSID: ", bestNetwork.ssid, ", Best dB: ", bestNetworkDb);
